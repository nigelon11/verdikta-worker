tweet drafts: three agent-wallet launches in five days, each graded "thin"
...
— thread opener —
5a. Cloudflare, Circle, and MetaMask each shipped agent-payment infrastructure within five days of each other. Every one of those days got filed as a quiet news day. Here's why that's backwards: [thread: Cloudflare/Circle/MetaMask specifics, zero coordination, convergence-not-hype]
5b. Three companies agreed on the same AI-agent wallet design in the same week, without talking to each other. Nobody called it the story. Here's what actually happened: [thread: same specifics, market-defines-the-spec close]

best: #4b — three unrelated companies independently converging on the same guardrails in one week is stronger evidence than any single launch, and the reframe (watch what ships, not what's called big) is the sharpest angle in the batch.
