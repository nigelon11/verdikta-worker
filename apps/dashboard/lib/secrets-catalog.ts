...
export const BUILTIN_SECRETS: Omit<Secret, 'isSet'>[] = [
  ...
  { name: 'VERDIKTA_WALLET_KEY', group: 'Skill Keys', description: 'Private key of a DEDICATED low-balance Base hot wallet - verdikta-hunter on-chain submissions only. Every tx is signed only by scripts/verdikta-exec.sh, pinned to the BountyEscrow contract and hard-capped by VERDIKTA_MAX_SPEND_ETH (default 0.0005, override in memory/verdikta-hunter.env). Never reuse a wallet holding real funds' },
