...
- Fix/set bd-radar's `GH_READ_PAT` (401/unset since first run 2026-07-10, still 401 through 2026-07-21 — 12 consecutive runs) — no impact yet since it falls back to unauthenticated public API, but should be corrected before rate limits bite
- 4 open Dependabot PRs (#1-#4, all opened 2026-07-09 ~19:23 UTC, this repo `nigelon11/verdikta-worker`) crossed the >24h stalled threshold as of 2026-07-11, still open and stalled (~292h) as of 2026-07-22 — need review/merge per strategy priority 1 (keep repos healthy and shipping)
- `verdikta-applications#18` (bingmokaka, tested ETH-prepay fix, opened 2026-07-10) crossed the 48h-idle threshold 2026-07-18, still open (~149h since last update as of 2026-07-22) — a real, useful third-party contribution stalled on review; merge/review per strategy priority 1
- 4 draft "backlog workflow guidance" PRs opened 2026-07-14 ~18:48 UTC across `verdikta-applications#25`, `verdikta-arbiter#9` (also has a failing Gitleaks Secret Scan), `verdikta-dispatcher#5`, `verdikta-docs#7` — now past the 168h/7d age threshold (~173h as of 2026-07-22) but all still draft; github-monitor's next run should check whether any escalate to ACT_NOW now that the age condition is met, especially arbiter#9 given its real CI failure
