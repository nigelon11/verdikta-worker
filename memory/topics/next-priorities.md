...
Remaining open:
- **Dependabot churn** (`nigelon11/verdikta-worker`, 2026-08-19 live check): `#9`/`#10` (opened 2026-08-01, ~451h old) not yet stalled; `#2`/`#3` (opened 07-09) remain open and are the longest-stalled at **~988h**
- **`verdikta-applications#18`** (bingmokaka, tested ETH-prepay fix, opened 2026-07-10) crossed the 48h-idle threshold 2026-07-18, still open (~844h since last update as of 2026-08-19) — a real, useful third-party contribution stalled on review
- **Draft "backlog workflow guidance" PR cluster**: `verdikta-docs#7` merged 2026-07-22, `verdikta-applications#25` merged 2026-08-19 — 2 of 4 resolved. Remaining: `verdikta-arbiter#9` (still failing Gitleaks Secret Scan) and `verdikta-dispatcher#5`, both ~869h as of 2026-08-19, sitting in github-monitor's INFO tier (draft-capped, not ACT_NOW); `arbiter#9` notable for its real, unresolved Gitleaks CI failure
- **`verdikta-applications#8`** (operator's own bundle-submit-flow skill PR, opened 2026-04-05) — touched 2026-08-19 (updatedAt bumped, same session as the merge spree) but still open, not merged; ~3265h since open
- **`verdikta-dispatcher!6`** (operator's own issue, opened 2026-07-29, "multi-identity operator failures under-penalized" oracle-selection concern) — surfaced by github-monitor 2026-07-30, still open and unaddressed (~507h as of 2026-08-19), long aged out of the 24h new-issue window but remains a watch item

## Repo scratch/tmp cleanup

Baseline 2026-08-04, re-checked daily. This repo (`verdikta-worker`) has scratch/tmp files committed to git — reproducible `git ls-files | grep -icE 'tmp'` count is **156** as of 2026-08-19, flat vs 08-18 (5th flat day of the last 6; prior deltas: +19 on 08-11, +2 on 08-12, 0 on 08-13, +4 on 08-14, 0 on 08-15, +13 on 08-16, 0 on 08-17, 0 on 08-18). See [MEMORY.md Lessons Learned](../MEMORY.md#lessons-learned) for the count-method history.
