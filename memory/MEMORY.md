...
- digest/write-tweet/github-monitor/bd-radar/shiplog scratch workarounds (xAI payloads, fork/issue JSON dumps, count scripts, notify bodies) recur most runs; `rm` is blocked by the sandbox destructive-op gate so memory-flush can't clean them itself. **Checking `git status` alone isn't enough to confirm they're harmless** — `git ls-files` (2026-07-30) found 103 of these already tracked/committed (`.runtmp/`, `.bdtmp/`, `.aeon-tmp/`, `.ghmon-tmp/`, `digest-tmp/`, `tmp/`, root-level `tmp-*`), none matching `.gitignore`'s `tmp_*`/`.*-tmp.md` patterns (naming mismatch: `tmp-`/`-tmp`/`.runtmp` vs the covered `tmp_`) — real repo bloat, no secrets found, needs an actual cleanup PR (see Next Priorities)

## Next Priorities
- Add `read:org` scope to the GitHub token used by github-monitor
- Fix/set bd-radar's `GH_READ_PAT` (401/unset since first run 2026-07-10, still 401 through 2026-07-30 — 21 consecutive runs) — no impact yet since it falls back to unauthenticated public API, but should be corrected before rate limits bite
- 4 open Dependabot PRs (#1-#4, all opened 2026-07-09 ~19:23 UTC, this repo `nigelon11/verdikta-worker`) crossed the >24h stalled threshold as of 2026-07-11, still open and stalled (~508h as of 2026-07-30) — need review/merge per strategy priority 1 (keep repos healthy and shipping)
- `verdikta-applications#18` (bingmokaka, tested ETH-prepay fix, opened 2026-07-10) crossed the 48h-idle threshold 2026-07-18, still open (~364h since last update as of 2026-07-30) — a real, useful third-party contribution stalled on review; merge/review per strategy priority 1
- Draft "backlog workflow guidance" PR cluster opened 2026-07-14 ~18:48 UTC: `verdikta-docs#7` **merged 2026-07-22** (first resolution), leaving 3 still open/draft — `verdikta-applications#25`, `verdikta-arbiter#9` (still failing Gitleaks Secret Scan), `verdikta-dispatcher#5` — all confirmed past the 168h/7d age threshold (~389h as of 2026-07-30) and sitting in github-monitor's INFO tier (draft-capped, not ACT_NOW) — still need operator review/merge per strategy priority 1, especially `arbiter#9` given its real, unresolved Gitleaks CI failure
- `verdikta-applications#26` (operator's own rubricCid fix, opened 2026-07-21) crossed github-monitor's >48h INFO floor on 2026-07-24, still open with no CI configured and no reviews (~218h as of 2026-07-30) — lower urgency as an operator-owned PR, but now aging alongside the rest of the queue
- `verdikta-applications#27`/`#28` (mumuzhong3, opened 2026-07-24 ~18:56-19:00 UTC) — real fixes for open issues #14 (AWARDED bounty red error) and #16 (rubric validator missing jury-node checks), both MERGEABLE, no CI configured, no reviews yet; crossed github-monitor's 48h INFO floor 2026-07-27 (~135h old as of 2026-07-30) — a genuine third-party contribution, review/merge per strategy priority 1; confirmed as a builder lead by bd-radar (see Builder Leads above)
- **Escalated (2026-07-30):** this repo (`verdikta-worker`) has **103 scratch/tmp files actually committed to git** (`git ls-files`), not just harmless untracked stray files as previously logged — spans bd-radar/digest/write-tweet/github-monitor/shiplog scratch output across `.runtmp/`, `.bdtmp/`, `.aeon-tmp/`, `.ghmon-tmp/`, `digest-tmp/`, `tmp/`, and root-level loose files. None match existing `.gitignore` patterns. Scanned, no secrets found, but this is real repo bloat needing an actual cleanup PR (`git rm` the paths + fix whichever skill(s) are running a broad `git add`/`git commit` that sweeps scratch dirs in, and/or extend `.gitignore` to cover `tmp/`, `*tmp/`, and `.*tmp*` naming) — see Lessons Learned
- `verdikta-dispatcher!6` (operator's own new issue, opened 2026-07-29, "multi-identity operator failures under-penalized" oracle-selection concern) — surfaced by github-monitor 2026-07-30, still fresh (~27h), just a watch item for now
